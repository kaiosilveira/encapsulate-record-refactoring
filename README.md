...
We will also need to introduce an `Organization` class. Our goal is to have all the access to `organization` going through this class.
...
Clients will see the function with a strikethrough line now, which is a good visual effect to discourage people from using it:
...
Moving on, we need to update the clients to use the class variation of the `organization`:
...
Finally, we can fold our data object into class instance fields in the `Organization` class, so our internal representation is more concise:
